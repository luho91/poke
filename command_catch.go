...
func commandCatch(cfg *config, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("No Pokemon to catch was provided!")
	}
...
func tryCatch(baseExperience int) bool {
	...
	catchChance := func (baseExp int) float64 {
		...
		normalized := (exp - minExp) / (maxExp - minExp)
		chance := maxCatch - (maxCatch-minCatch)*math.Pow(normalized, gamma)
	...
	chance := catchChance(baseExperience)
	roll := rand.Intn(100)
